...
@Component({
  selector: 'tr[app-budget-profit-loss-row]',
  imports: [],
  templateUrl: './budget-profit-loss-row.component.html',
  styles: ``,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class BudgetProfitLossRowComponent {
  public readonly months = input.required<MonthYear[]>();
  public readonly totals = input.required<BudgetTotals>();

  protected readonly profitLossValues = computed(() => {
    return this.months().map((month) => {
      const key = getMonthKey(month);
      return this.totals().profitLoss.get(key) || 0;
    });
  });
